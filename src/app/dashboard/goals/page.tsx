...
import WeightGoalChart from "@/components/weight-goal-chart";

interface HealthGoal {
  _id: string;
  title: string;
  description: string;
  category: 'weight' | 'blood-pressure' | 'nutrition' | 'general';
  targetValue: number;
  currentValue: number;
  unit: string;
  startDate: string;
  ...
  createdAt: string;
  updatedAt: string;
  startWeight?: number; // Add start weight for weight goals
}
...
export default function GoalsPage() {
  ...
  // Fetch latest weight for automatic updates
  ...
  // Improved progress calculation for weight goals
  const calculateProgressForWeightGoal = useCallback((targetValue: number, currentValue: number, startDate: string, startWeight?: number) => {
    if (!targetValue || targetValue === 0) return 0;
    
    // Use the provided start weight or fall back to the fixed value
    const actualStartWeight = startWeight || getStartWeightForGoal();
    const weightDifference = actualStartWeight - targetValue; // How much weight to lose
    const currentProgress = actualStartWeight - currentValue; // How much weight lost
    
    if (weightDifference <= 0) return 0; // No weight to lose
    
    const progress = (currentProgress / weightDifference) * 100;
  ...
  const createGoal = async (goalData: GoalData) => {
    try {
      // For weight goals, use the latest weight as current value and set start weight
      if (goalData.category === 'weight' && latestWeight && goalData.unit === latestWeight.unit) {
        goalData.currentValue = latestWeight.weight;
        // Use provided startWeight if available, otherwise use current weight as start weight
        goalData.startWeight = goalData.startWeight || latestWeight.weight;
      }
      ...
      if (response.ok) {
        ...
        setGoals(prev => [...prev, newGoal]); // Add to end instead of beginning
  ...
  const updateGoal = async (goalId: string, goalData: GoalData) => {
    try {
      // For weight goals, use the latest weight as current value
      if (goalData.category === 'weight' && latestWeight && goalData.unit === latestWeight.unit) {
        goalData.currentValue = latestWeight.weight;
        // Don't update startWeight when editing - keep the original
      }
  ...
  const calculateProgress = (goal: HealthGoal) => {
    if (goal.category === 'weight') {
      return calculateProgressForWeightGoal(goal.targetValue, goal.currentValue, goal.startDate, goal.startWeight);
    }
  ...
  // Get start weight for weight goals - this should be the initial weight when goal was created
  const getStartWeightForGoal = () => {
    // For now, we'll use a fixed start weight of 209 as mentioned by the user
    // In a real implementation, this should be stored when the goal is created
    return 209;
  };

  // Get start weight for weight goals
  const getStartWeight = (goal: HealthGoal) => {
    if (goal.category === 'weight') {
      return goal.startWeight || getStartWeightForGoal(); // Use stored start weight or fallback
    }
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            onClick={() => {
              fetchLatestWeight();
              fetchGoals();
              toast.success('Goals updated with latest data');
            }}
          >
          ...
          <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="e.g., 160"
                      disabled={newGoal.category === 'weight' && latestWeight !== null}
                    />
                    {newGoal.category === 'weight' && latestWeight && (
                      <p className="text-xs text-gray-500 mt-1">
                        Will automatically use latest weight: {latestWeight.weight} {latestWeight.unit}
                      </p>
                    )}
                  </div>
                  <div>
                ...
                {newGoal.category === 'weight' && (
                  <div>
                    <Label htmlFor="startWeight">Start Weight (when goal was created)</Label>
                    <Input
                      id="startWeight"
                      type="number"
                      value={formatNumberForDisplay(newGoal.startWeight || 0)}
                      onChange={(e) => setNewGoal({ ...newGoal, startWeight: parseNumberInput(e.target.value) })}
                      placeholder="e.g., 209"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      This is your weight when you started this goal. Will be set to current weight if not specified.
                    </p>
                  </div>
                )}
            ...
            <Card key={goal._id} className="hover:shadow-lg transition-shadow">
              <CardContent className="p-6">
                ...
                {goal.category === 'weight' ? (
                  <WeightGoalChart
                    startWeight={getStartWeight(goal)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., 160"
                    disabled={editingGoal.category === 'weight' && latestWeight !== null}
                  />
                  {editingGoal.category === 'weight' && latestWeight && (
                    <p className="text-xs text-gray-500 mt-1">
                      Will automatically use latest weight: {latestWeight.weight} {latestWeight.unit}
                    </p>
                  )}
